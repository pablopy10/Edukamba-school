...
import { backfillTuitionFeesForRuleClient } from "@/lib/finance/backfillRecurringCharges";
import {
  ...
  const saveRule = async () => {
    ...
    if (activeYearId) {
      ...
      const savedRule: FeeRule = {
        ...
        generate_all_upfront: payload.generate_all_upfront,
        target_scope: payload.target_scope,
        notes: payload.notes,
        fee_rule_classrooms:
          ruleForm.target_scope === "classrooms"
            ? ruleForm.classroom_ids.map((cid) => ({ classroom_id: cid }))
            : [],
        fee_rule_students:
          ruleForm.target_scope === "students"
            ? ruleForm.student_ids.map((sid) => ({ student_id: sid }))
            : [],
      };
      const eligStudents = studentsMatchingFeeRule(savedRule, activeYearId, students, classrooms);
      ...
      const clientCount = await backfillTuitionFeesForRuleClient(
        supabase,
        savedRule,
        activeYearId,
        yearStart,
      ...
      if (generatedCount > 0) {
        toast({
          ...
          description: `${generatedCount} cobrança(s) criada(s) automaticamente (meses anteriores e corrente).`,
        });
      }
    }
