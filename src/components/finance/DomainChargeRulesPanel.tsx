...
import { backfillDomainFeesForRuleClient } from "@/lib/finance/backfillRecurringCharges";
...
export function DomainChargeRulesPanel({ variant, schoolId, role }: Props) {
  ...
  const saveRule = async () => {
    ...
    if (ruleId && supportsRuleDetail && activeYearId) {
      const domain =
        variant === "activity" ? "activity" : variant === "transport" ? "transport" : "meal";
      const yearStart = years.find((y) => y.id === activeYearId)?.start_date?.slice(0, 10);
      const savedRule = {
        id: ruleId,
        ...base,
        generate_all_upfront: base.generate_all_upfront,
      };
      ...
      if (variant === "activity") {
        const { data } = await supabase
          ...
          .select("student_id")
          .eq("school_id", schoolId)
          .eq("activity_id", ruleForm.entity_id);
        (data ?? []).forEach((row: { student_id: string }) => enrolledIds.add(row.student_id));
      } else if (variant === "transport") {
        const { data } = await supabase
          .from("transport_enrollments")
          .select("student_id")
          .eq("school_id", schoolId)
          .eq("route_id", ruleForm.entity_id);
        (data ?? []).forEach((row: { student_id: string }) => enrolledIds.add(row.student_id));
      } else if (variant === "meal") {
        const { data } = await supabase
          .from("meal_enrollments")
          .select("student_id")
          .eq("school_id", schoolId)
          .eq("meal_program_id", ruleForm.entity_id);
        (data ?? []).forEach((row: { student_id: string }) => enrolledIds.add(row.student_id));
      }

      const eligStudents = studentsMatchingDomainChargeRule(
        savedRule as DomainChargeRuleRow,
        activeYearId,
        enrolledIds,
        students,
        classrooms,
      ...
      if (!backfillErr) {
        generatedCount = (autoGenerated as number | null) ?? 0;
      }
      const clientCount = await backfillDomainFeesForRuleClient(
        supabase,
        domain,
        savedRule,
        activeYearId,
        yearStart,
        eligStudents.map((s) => s.id),
      );
      generatedCount += clientCount;
      if (backfillErr && generatedCount <= 0) {
        toast({
      ...
      if (generatedCount > 0) {
        toast({
          ...
          description: `${generatedCount} cobrança(s) criada(s) automaticamente (períodos em atraso e mês corrente).`,
        });
      }
    }
