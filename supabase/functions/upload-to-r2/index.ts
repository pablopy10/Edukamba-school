/**
 * Upload via servidor → R2 (evita CORS do PUT presigned no browser/Capacitor).
 * Máx. 5 MB por pedido (limite típico de Edge Functions).
 *
 * POST corpo binário + headers:
 *   x-file-name, x-file-type, x-prefix (opcional)
 */
...
const corsHeaders: Record<string, string> = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-file-name, x-file-type, x-prefix",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prefix = (req.headers.get("x-prefix")?.trim() || "documents").replace(/^\/+|\/+$/g, "");
    const schoolId =
      profile?.school_id ??
      (PREFIXES_WITHOUT_SCHOOL.has(prefix) ? "onboarding" : null);
    ...
    const fileType = req.headers.get("x-file-type")?.trim() || "application/octet-stream";

    if (!fileName) {
      return corsJson({ error: "x-file-name header is required" }, 400);
    }
    if (!ALLOWED_PREFIXES.has(prefix)) {
      return corsJson({ error: `prefix not allowed` }, 400);
    }

    const body = await req.arrayBuffer();
    if (body.byteLength === 0) {
      return corsJson({ error: "Empty file body" }, 400);
    }
    if (body.byteLength > MAX_BYTES) {
      return corsJson(
        { error: `File too large for server upload (max ${MAX_BYTES} bytes). Use presigned upload.` },
        413,
      );
    }
